...
for i=1:7
%função para pegar um valor aleatório de dAlphacor que é Coeficiente de correlação do sombreamento entre ERBs (sombreamento correlacionado)
dAlphaCorr = rand (1);                                    
...
% Resolução do grid: distância entre pontos de medição
dPasso = 10;
...
DevPatMtshd = std(mtShadowing(:)); %Função que calcula o desvio padrão da matriz mtShadowing
IntDevPatMtshd = round(DevPatMtshd); %Como o valor dá sempre aproximado por ser real, aqui pega apenas o inteiro mais próximo.
ERB = round(i); %Um truque para pegar o valor da iteração i que tá alterando o dAlphacorr
disp(['O valor de dAlphacorr é: ' num2str(dAlphaCorr)]);
disp(['O valor do Desvio padrão de mtShadowing é ' num2str(DevPatMtshd)]);
disp(['O valor do Desvio padrão de mtShadowing no inteiro mais próximo é ' num2str(IntDevPatMtshd)]);
if IntDevPatMtshd == dSigmaShad %Função para comparar se o desvio padrão correlacionado está sendo igual ao dSigmaShad
   disp(['O valor é igual na = ' num2str(ERB) 'a. ERB']);
   disp(['##############################################']);
else
   disp(['O valor é diferente na = ' num2str(ERB) 'a. ERB']);
end
end
