...
for iFc = 1:length(vtFc)
    for dR=1e3:0.01e3:10e3   %For para fazer o raio variar lentamente de 1km até 10km
    dFc = vtFc(iFc);
    ...
    dOffset = pi/6;
        ...
        % Calculo do Outage (limite 10%)
        dOutRate = 100*length(find(mtPowerFinaldBm < dSensitivity))/numel(mtPowerFinaldBm); 
        %Condição para testar o outage e armazena esses valores na
        %variável;
        if dOutRate <= 10;
            Outag = dOutRate;
            Raio = dR;
    ...
    disp(['Frequência da portadora Okumura= ' num2str(Freq)]);
    disp(['Raio Aproximado Okumura= ' num2str(Raio)]);
    disp(['Taxa de Outage Okunura= ' num2str(Outag)]);
end